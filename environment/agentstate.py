    ...
    def forward(self, gridwidth, gridheight):
        if self.orientation.name == Orientation.East.name:
            self.location = Coords(
                min(gridwidth-1, self.location.x+1), self.location.y)
            ...
            self.location = Coords(self.location.x, min(
                gridheight-1, self.location.y+1))

    def show_state(self):
        print("Location:", self.location, "orientation:", self.orientation, "hasGold:",
              self.has_gold, "hasArrow:", self.has_arrow, "isAlive:", self.is_alive)
...
class Percept():
    ...
    def show_percept(self):
        print("stench:", self.stench, "breeze:", self.breeze, "glitter:", self.glitter, "bump:",
              self.bump, "scream:", self.scream, "isTerminated:", self.isTerminated, "reward:", self.reward)
